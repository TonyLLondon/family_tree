...
4. Edge `web/proxy.ts`: `.md`/`.yaml` under `/files/` rewrite to `/view/…` for a wrapped reader (local dev). **On Vercel** (`VERCEL=1`), corpus + media under `/files/…` skip that rewrite so the static copy is served—serverless `/view` has no full repo. Append `?raw` to force raw `/files/…` anywhere.

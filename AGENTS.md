...
| `projects/` | **Per-project operational folders.** Each is a self-contained workflow with its own `AGENTS.md` (playbook), `state.md` (live dashboard), `inbox/` (gitignored raw drops), `runner/` (scripts), `history/logs.md` (project changelog). Outputs land in canonical repo paths (`sources/corpus/`, `media/`, …). First instance: [`projects/ingest_source/`](projects/ingest_source/AGENTS.md) — source intake + transcription pipeline (probe → Tier A text-layer / Tier B full render / Tier C selective hits + `snippets/` → vision pass → publish). New long-running workflows should follow the same pattern. |
